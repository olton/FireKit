...
/******************************************************************************
 *                                                                            *
 *   Calc.php, v 0.00 2007/09/24 - the part of phpNatty CMS                   *
 *   Copyright (C) 2007  Dmitriy Skorobogatov  info@xbb.uz                    *
 *                                                                            *
...
// Класс - формульный калькулятор
// Написан с помощью алгоритмов обратной польской записи и Дейкстры.
class Calc
{
    // Стековый приоритет операций
    var $_priors = array(
        '('  => 0 , // Левая скобка
        ')'  => 1 , // Правая скобка
    ...
    */
    ...
    // Записывает ошибку
    private function _setError($num, $str = '')
    {
    ...
    */
    private function _parse($formula)
    {
        $polish = array();
        if (! strlen($formula)) {
            $this->_setError(1);
    ...
    public function getResult($formula)
    {
    ...
    private function _getRange($var_name1, $var_name2)
    {
    ...
    private function _getIntersection($range1, $range2)
    {
    ...
    private function _getColNum($col_str)
    {
    ...
    private function _arrgsToArray($args)
    {
        if (! is_array($args)) {
            return array($args);
        }
    ...
    private function _abs($num)
    {
        return abs((float) $num);
    }

    // ACOS - Arc cosine.
    private function _acos($num)
    {
    ...
    // ATAN - Arc tangent.
    private function _atan($num)
    {
        return atan((float) $num);
    }

    // ATAN2 - Arc tangent of two variables.
    private function _atan2($args)
    {
        if (! is_array($args)) {
            return NAN;
        }
    ...
    private function _combina($args)
    {
        $args = $this->_arrgsToArray($args);
        if (! is_array($args) || 2 > count($args)) {
            return null;
    ...
    // COS - Cosine.
    private function _cos($num)
    {
        return cos((float) $num);
    }

    // COSH - Hyperbolic cosine.
    private function _cosh($num)
    {
        return cosh((float) $num);
    }

    // COT - Возвращает котангенс угла.
    private function _cot($num)
    {
    ...
    private function _countblank($range)
    {
    ...
    */
    ...
    private function _factDouble($num)
    {
        $num = (int) $num;
        if (0 > $num) {
            return NAN;
    ...
    */
    ...
    private function _log10($num)
    {
        return log10((float) $num);
    }

    // MIN - минимум
    private function _min($args)
    {
        if (! is_array($args)) {
            return (float) $args;
        }
    ...
    */
    ...
    // PI (ПИ) - число pi
    private function _pi()
    {
        return M_PI;
    }

    // POWER - Возвращает число, возведенное в степень.
    private function _power($args)
    {
        if (! is_array($args)) {
            return NAN;
        }
    ...
    private function _product($args)
    {
        if (! is_array($args)) {
            return (float) $args;
        }
