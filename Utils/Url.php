...
class Url extends \FireKit\Base\Super{
    ...
    public static function Pager($all = 0, $on_page = 10, $distance = 6, $captions = array()){
        ...
        // Всего страниц
        $pager[] = "<form style='float: right;'><span>{$captions['pages']}: <strong>$current / $pages</strong></span>";
        ...
        $pager[] = "<ul>";
        if ($current > $start) {
            $pager[] = "<li class='prev'><a href='/".$cUri."/".($current-1)."'>&larr; {$captions['prior']}</a></li>";
            ...
            $pager[] = "<li class='prev disabled'><a href='#' onclick='return false;'>&larr; {$captions['prior']}</a></li>";
        }
        ...
        // Next
        if ($current < $pages) {
            $pager[] = "<li class='next'><a href='/".$cUri."/".($current+1)."'>{$captions['next']} &rarr;</a></li>";
        } else {
            $pager[] = "<li class='next disabled'><a href='#' onclick='return false'>{$captions['next']} &rarr;</a></li>";
        }
        $pager[] = "</ul>";
        return join("\n", $pager);
    }
