<?php
namespace FireKit\Database\Provider\MySQL;

use \FireKit\Base\Super;
use \FireKit\Database\Interfaces\IDataProvider;
use \FireKit\Exceptions\DataProviderException;
 ...
 */
class MySQLDataProvider extends Super implements IDataProvider {
    private $queries;
    private $stack;
    private $conn;
    private $query;
    ...
    public function __construct($config){
        ...
        $this->fetch = isset($config['fetch'])?$config['fetch']:MYSQL_ASSOC;
    ...
    public function SetFetchMode($mode = MYSQL_ASSOC){
        $this->fetch = $mode;
    }
    ...
    private function _execSQL($sql){
        ...
        if ($this->query === false) {
            throw new DataProviderException(MSG_DATABASE_PROVIDER_SQL_ERROR . " [".(mysql_errno($this->conn))."] " . (mysql_error($this->conn)), E_USER_ERROR);
        }
        
        $sql_type = strtoupper(substr($sql, 0, strpos($sql, " ")));

        $rows = ($sql_type == "SELECT") ? mysql_num_rows($this->query) : mysql_affected_rows($this->conn);
    ...
    public function Rows($handle = false){
        ...
        return ($handler === true) ? mysql_affected_rows($handler) : mysql_num_rows($handler);
    }

    public function Columns($table){
        $columns = array();
        $current_fetch_mode = $this->fetch;
        $this->SetFetchMode(MYSQL_ASSOC);
        $sql = "DESC $table";
        $h = $this->Select($sql);
    ...
    public function FetchArray($handle = false){
        ...
        return mysql_fetch_array($handle, $this->fetch);
    }
    
    public function FetchObject($handle = false, $class = false){
        $handle = $handle?:$this->query;
        return mysql_fetch_object($handle, $class);        
    }
    ...
    public function ExecProc($name, $params){
        $temp_conn = mysql_connect($this->host, $this->user, $this->password, true, CLIENT_MULTI_STATEMENTS);
        if (!$temp_conn) throw new DataProviderException(MSG_DATABASE_PROVIDER_NO_CONNECT, E_USER_ERROR);
        $res = mysql_select_db($this->schema, $temp_conn);
        if (!$res) {
            mysql_close($temp_conn);
            throw new DataProviderException(MSG_DATABASE_PROVIDER_SQL_ERROR, E_USER_ERROR);
        }
        mysql_query("set names {$this->charset}", $temp_conn);
        if (!empty($params)) {
    ...
    public function ExecFunc($name, $params){
        ...
        $query = "select $name($_p) as result";
        $this->queries[] = $query;
        $h = mysql_query($query, $temp_conn);
        $result = mysql_fetch_assoc($h);
        mysql_close($temp_conn);
    ...
    public function GetCreateDDL($object){
        ...
        if (isset($object['indexes']) && !empty($object['indexes']))
            foreach($object['indexes'] as $index=>$struct){
                $indexes[] = $this->_compose_index($index, $struct);
        }
    ...
    public function GetUpdateDDL($object){
        ...
        if ($new['indexes']) foreach($new['indexes'] as $type=>$struct){
            $changes[] = "ADD " . $this->_compose_index($type, $struct);
        }
